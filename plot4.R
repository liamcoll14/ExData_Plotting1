...
##Change par to present 2 x 2 plots
par(mfcol = c(2, 2))

##Create 4 plots in order
##Top left
...
##Top right
with(hpc2, {plot(datetime, Voltage, type="l", xlab=NA, xaxt="n", ylab="Voltage (volts)")
        ##Plot not auto assigning days on x axis so add manually
        axis(1, at = c(datetime[1], datetime[1440], datetime[2880]), labels = c("Thu","Fri","Sat"))})
    

##Bottom right
with(hpc2, {plot(datetime, Global_reactive_power, type="l", xlab=NA, xaxt="n", ylab="Global reactive power (kilowatts)")
  ##Plot not auto assigning days on x axis so add manually
  axis(1, at = c(datetime[1], datetime[1440], datetime[2880]), labels = c("Thu","Fri","Sat"))})    
